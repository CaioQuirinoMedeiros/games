...
renderWall = () =>
  game.wall
    ? stage.classList.add("stage-wall")
    : stage.classList.remove("stage-wall");
...
calcDist = () => {
  ...
  return Math.round(
    Math.sqrt(Math.pow(food.distX, 2) + Math.pow(food.distY, 2))
  );
};
...
timer = () => {
  ...
  timerBox.style.color = `hsl(${(100 * snake.contador) /
    snake.tempo}, 100%, 70%)`;
};

/***** Get points *****/
getPoints = () => {
  snake.points += Math.ceil(
    (snake.contador / snake.tempo) * 10 * multiplicador
  );
  renderPoints();
};
/***** Render points *****/
...
makeEyes = () => {
  ...
  if (vel.y) {
    game.context.fillRect(
      snake.x * game.pixel + 3,
      snake.y * game.pixel + 9,
      3,
      3
    );
    game.context.fillRect(
      snake.x * game.pixel + 13,
      snake.y * game.pixel + 9,
      3,
      3
    );
    ...
    game.context.fillRect(
      snake.x * game.pixel + 9,
      snake.y * game.pixel + 3,
      3,
      3
    );
    game.context.fillRect(
      snake.x * game.pixel + 9,
      snake.y * game.pixel + 13,
      3,
      3
    );
  }
...
const summaryRender = () => {
  summaryDifficulty.textContent =
    game.dificuldade.charAt(0).toUpperCase() + game.dificuldade.slice(1);
...
window.addEventListener("load", () => {
  ...
  dificuldade.textContent = document.getElementById(
    game.dificuldade
  ).textContent;
